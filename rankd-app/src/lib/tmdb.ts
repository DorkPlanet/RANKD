...
/**
 * Frames from a film, cleanest first.
 *
 * ── The sort is the whole point ────────────────────────────────────────────
 *
 * Lifted from `app/api/stills/route.ts`, whose comment explains it: asking TMDb
 * for its best-voted backdrops gets you promotional art with the title baked
 * across it, which is a poster by another name. Textless images come first, and
 * only then does the vote average decide.
 *
 * That matters more here than it did there. A profile banner sits under
 * somebody's name, so a frame with its own title across it puts two competing
 * pieces of type in the same place.
 *
 * Returns [] rather than throwing. A banner is decoration: a film TMDb has no
 * art for should fall back quietly, never fail a page.
 */
export async function backdropsFor(tmdbId: number, key: string): Promise<string[]> {
  ...
  try {
    const res = await fetch(url, { next: { revalidate: DAY } });
    if (!res.ok) return [];
    const backdrops: { file_path: string; iso_639_1?: string | null; vote_average?: number }[] =
      (await res.json())?.backdrops ?? [];
