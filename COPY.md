...
| 21 Aug | Everywhere | **The "settled" → "locked" sweep finished.** The King of the Hill blurb ("a place you settled yourself"), its picker label (SETTLES FILMS → LOCKS FILMS), the list tour's Jump step, and the row tooltip. Found by grepping the LIVE bundle after deploying, which is what that step is for — three of them had survived the pass that renamed the legend |
| 21 Aug | Fast Shuffle setup | **"HOW LONG" → "HOW MANY".** 25/50/100 films with an ETA beneath ("50 films to get a number · about 15 min") instead of a length of time. The app states the size of the job rather than asking how long you want to play |
...
| 21 Aug | Film card | Settledness is a **percentage plus a word** ("62% taking shape"), replacing three words alone. The old comment argued a percentage could not be honest because confidence saturates — true of raw confidence, and fixed by scaling to the measured ceiling |
| 21 Aug | List legend, film card, Settings | **"Rankd placed" → "shuffled" / "Fast Shuffle placed".** The user's call, and checkable rather than a matter of taste: `placeSettled` is called from exactly one component, so a soft lock can only ever have come from Fast Shuffle. Naming the mode says where the number came from and where to go to change it. `Rankd says #38` on the film card is left alone — that is the model's opinion about a position, not a claim about who placed it |
