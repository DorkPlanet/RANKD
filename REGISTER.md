...
| P12 | **Pin a handful of cards to the top of the profile.** The user's ask, 21 Aug: cards you have made should be pinnable to the profile, sitting at the top and big enough to read. Needs somewhere on the profile to store the chosen ids, a picker (`CardPicker` exists), and a block above the stats band. **Sits with P11** — both are the profile becoming something you act on rather than only read, which is the through-line of every profile note in this section.<br><br>**DISTINCTIVENESS — read before building this.** Letterboxd puts **FAVORITES**, four hand-picked posters, directly under the bio. "Hand-picked things pinned near the top of a film profile" is that block. Built as a row of poster tiles it lands on their most recognisable element; built as **your ranking artefacts** — a card you made, carrying its rank and its count — it does not, because they have nothing like it. Same feature, and the execution decides whether it reads as ours or theirs | NEW, OPEN |
...
| P16 | **Pin favourite directors and actors to the Your results tab.** The user's ask, 21 Aug. Sits directly with **P12** (pin cards to the profile) — same gesture, same surface, same storage question — and with **P11** (gold highlights should open their films). All three are the profile turning from something you read into something you arrange. `topPeople` already derives the candidates and `PersonSheet` already opens one, so the missing pieces are storage on the profile and a picker. <br><br>**DISTINCTIVENESS — read before building this.** Letterboxd puts **FAVORITES**, four hand-picked posters, directly under the bio. "Hand-picked things pinned near the top of a film profile" is that block. Built as a row of poster tiles it lands on their most recognisable element; built as **your ranking artefacts** — a card you made, carrying its rank and its count — it does not, because they have nothing like it. Same feature, and the execution decides whether it reads as ours or theirs. The question was raised with two screenshots side by side and the answer was that the layout is a convention nobody owns, but that THESE two entries are where the real resemblance would come from | NEW, OPEN |
