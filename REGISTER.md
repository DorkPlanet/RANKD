...
| B5 | ~~**Profile page redesign.**~~ **DONE 21 Aug 2026, and superseded by the P-series that tracks the specifics.** What landed: panels renamed and reordered so **Your results** is what you land on (P3, P17), the shelves became a two-up grid (P4), the de-boxing finished (P5), the type scale reached it (P6/P7), "Last time" went (N18), the sections reordered from most personal to most factual, the bio took 300 characters with line breaks, every fact became a way into its films (P11), and people became pinnable (P16). **What is still open lives in its own entries rather than under this one:** P12 pinned cards, P13 desktop, P1 the world map (parked), B8's design pass for `PersonSheet`, and C13's narrowed remainder. Kept as a closure rather than deleted, because "redesign the profile" is the kind of entry that stays open forever unless somebody says when it was finished | DONE |
| B7 | ~~**Resume an in-progress curated run.**~~ **DONE 21 Aug 2026.** `runs.ts` gains a second store under `rankd-run-curated-v1`, holding the session, the subject and the **guests as whole films**. That last part is the entire reason it could not share the climb store: a climb rebuilds from ids because every film in it is in your library, while a curated run borrows work you have never seen and a guest exists nowhere else — so validation counts guests as present, and checking against the library alone would have silently discarded every run that borrowed anything, which is most of them. **The design was anticipated in the codebase:** `subjectKey` already described itself as "the primary key of the resume store". Saved on the same single commit path as the climb, and the two stores are mutually exclusive by construction — each clears when handed the other kind. Device-local: in neither `SYNC_KEYS` nor the backup, because an unfinished pile is working state rather than a judgement, and this one additionally holds films the other device has never heard of. Nine tests | DONE |
...
| C2 | ~~**Fast Shuffle as the lazy mode.**~~ **DONE 21 Aug 2026.** The payoff exists: the readout reports how many films the model has WORKED OUT rather than how many it has merely duelled — the old measure saturated after one duel per film, which is the point at which it has worked out nothing. A batch run bounds it, the anchor makes it move (N12), and the confidence question the entry ends on is answered too: `settledness()` scales between the confidence a film has when placed and the MEASURED ceiling, so the film card shows a percentage that genuinely reaches 100% (N13). The entry's own warning — that a raw figure would stall near 80% and read as broken — is exactly why it is scaled rather than raw | DONE |
| C3 | ~~**Rough Cut: placing a film blind.**~~ **DONE 21 Aug 2026, using the fix this entry proposed.** The last two films filed into each pile now sit as small posters directly above that pile's button. The problem was real and the mode otherwise hides it well: there is no opponent, so "upper or middle" is answered against a standard you hold in your head — and by film forty that standard has drifted from film four with nothing on screen to catch it. **The rejected alternative stays rejected for the reason recorded here:** four films and multi-select costs the one-tap-per-film economics the whole mode is built on and turns it into a small sort, which is King of the Hill with extra steps. Fitted INSIDE the counts row rather than above it — this screen divides its height by weight and a new row would take that from the poster, which is the one thing you actually have to look at. The most recent of the two is drawn taller and brighter, so the pair reads as "the last one, and the one before it" rather than as a list | DONE |
...
| C8 | **Consistency and break-it pass.** ~~Rough Cut has no gesture for Middle. Fast Shuffle silently ignores flicks.~~ **BOTH FIXED 21 Aug 2026, and the second was worse than the entry said.** Fast Shuffle did not silently ignore a flick — it played the full fly-away animation and THEN called a no-op, so the app appeared to accept a throw and quietly refused it. `onFlick`/`onSink` are optional now and the card springs back when a mode has nowhere to throw a film to; falling through to `onPick` was rejected, because a deliberate throw is not a vote and turning one into a pick would answer a duel nobody asked. Rough Cut's Middle got the one gesture that was free: **a tap on the card**. Not a horizontal flick — that card sets `touchAction: "pan-x"`, so sideways belongs to the browser. Tap is right for it anyway: up and down are claims, and Middle is the absence of one. The tour said "Middle has no flick, tap it" and now teaches three gestures instead of two and an exception.<br><br>**Still open, and it is the half that needs a decision:** two `Sheet` implementations plus three hand-rolled copies — `ui.tsx` and `Sheet.tsx` behave differently on dismissal, and `CollectionSheet`, `EditIdentity` and `AvatarCropper` each repeat the markup. Consolidating changes how these screens look, so it is a design call before it is a refactor | GESTURES DONE · SHEETS OPEN |
...
| C13 | ~~**The profile shows full density from film two.** Every screen has a zero state; nothing has a **thin** state.~~ **DONE 21 Aug 2026.** The charts already gated themselves — nothing below three axes, no locked shape below ten locks, no genre ring below three genres, nobody named on two films. The remaining gap was `superlatives`, which had NO minimum: a library of four produced "Oldest" and a biggest year with the same confidence as a library of nine hundred, and naming the oldest of four is a sort rather than a discovery. Gated at ten films to match what `notes.ts` already does at 8/10/20/40, plus two claim-specific bars: a "biggest" year needs three films or it only tied and won on sort order, and "most argued over" needs three duels or it is calling one duel an argument. Five tests | DONE |
...
| C19 | ~~**"Finishing a run doesn't feel like anything."**~~ **DONE 21 Aug 2026.** A run now HAS a finish: a Fast Shuffle batch ends itself when its films have their numbers, and `SessionEnd` offers the sizes again rather than one button back to a setup sheet. The climb and Rough Cut already ended in a summary. The original note said "partly answered by the cards, re-judge on a phone" — the missing half was that two of the four modes never ended at all | DONE |
| C20 | ~~Known wart: a brand-new phone hits the conflict chooser rather than a silent pull.~~ **ALREADY FIXED, and this entry was the stale one.** `store.ts` carries `isUntouchedSeed` and `hasRealLibrary`, and `sync.ts` asks the latter rather than whether the key exists. The comment on it describes this exact bug in these words: *"the credits sweep wrote them to storage within seconds — so by the time anyone signed in, the key was there and a naive check called it a real library. On a new phone that produced a conflict."* Found while going to fix it | DONE — was already done |
| C21 | ~~`src/lib/sync.ts` shows as **binary in git diffs**.~~ **DONE 21 Aug 2026.** The diagnosis stands and is worth keeping: the two NUL bytes are DELIBERATE — `fingerprint` joins each key and value with a literal `\0` so a value containing the delimiter cannot forge a boundary — and deleting them would change every sync hash the app has ever computed. A cleanup script in this session nearly did. **Fixed the readable-diff half instead**, with a `.gitattributes` marking the file as text; verified by `git check-attr diff` returning `diff: set`. The file's history is legible and the bytes are untouched | DONE |
...
| E3 | ~~**Milestone with a forward pointer.** 34 badges currently show only what is earned.~~ **DONE 21 Aug 2026.** One line under the trophy case names the unearned badge you are closest to, with its progress. **Nearest by FRACTION, not by remainder** — ten films short of Collector (100) and ten short of Archivist (500) are not the same distance, and picking by "ten either way" would offer whichever sorted first. Badges at zero are excluded: 0 of 1,000 is not something you are close to, and offering it would be the app inventing an ambition on your behalf.  `Achievement` gained `have`/`need` beside the display string, because working out which is nearest by parsing "312 of 500" back out of text would be inventing a data format to avoid carrying two integers. **What this entry also wanted and did NOT get:** naming the actual move ("Rough Cut clears three-star in one pass"). That is per-badge advice copy for 34 badges and a writing job rather than a build — worth doing, worth doing deliberately | DONE — the copy half is E9 |
| E9 | **Name the MOVE on the next badge, not just the target.** Split out of E3, which shipped the pointer itself. Today the line reads "Settle 500 films · 312 of 500", which says what and not how. E3 argued Rankd could say "Rough Cut clears three-star in one pass" — richer, and it is 34 pieces of advice copy that each need to be true of the badge they sit on, so it belongs with the voice work rather than in a component | NEW, OPEN |
| E4 | ~~**Standing counts row** — Films / Placed / Un-rnkd.~~ **DONE 21 Aug 2026, by P15.** The list band is exactly this and one column better: **films · locked · shuffled · un-rnkd**, since "placed" turned out to be two different states worth separating. Each is also a filter (P14). Its promise to be the thin-state answer for C13 is what closes that one too | DONE |
...
| E8 | ~~The library row shows a raw Elo number in both reference apps. Rankd's slot is better spent on **who decided**.~~ **DONE 21 Aug 2026.** The row's numeral carries it in colour rather than in a second figure: gold and bold for a film you locked, `--accent` for one Fast Shuffle placed, dim UN-RNKD for one with no position. The band above names the three so the colours are a key rather than a code | DONE |
...
| N11 | ~~**A library-scoped Fast Shuffle run cannot move its own readout, and no readout can fix that.**~~ **ANSWERED 21 Aug 2026 by N12, and the entry was right about the wrong thing.** It concluded no readout could fix it, which was correct — the fix was the MATCHMAKER. Holding an anchor concentrates evidence where placements come from, and the same simulation that produced this entry's zero then placed all 865 films in 4,000 duels. What survives is its recommendation: a big library still wants a scope, and the setup sheet now states the size of the job so choosing All films is an informed choice | DONE — via N12 |
...
| P11 | ~~**The profile's gold highlights should open the films behind them.**~~ **DONE 21 Aug 2026.** Every taste line named a set of films and doing nothing about it — you could read that you own a hundred and forty documentaries and have no way to see them. The VALUE is now the control, not the row: the label is a question ("Your decade") and only the answer has films behind it. Underlined in the border colour rather than given a chevron, since this page is deliberately short of objects. **The genre list rows and the world flags open too. The RING itself deliberately does not** — a slice for a four-film genre is a couple of degrees of arc, and asking a thumb to hit it is asking it to miss; the list pane is the same data with targets you can land on, which is most of why it exists. **One line is deliberately NOT a way in:** "as a rater you are even-handed" describes HOW you rate rather than WHAT you rated, so there is no set behind it, and a control that opened the whole library would not be an answer | DONE |
...
| I2 | ~~**Contradiction to resolve** about `rankd-lists-v1` and backups.~~ **RESOLVED 21 Aug 2026, and the answer is better than either claim.** Saved rankings are in the FILE set (`backupFormat.ts` — `FILE_2` lists `rankd-lists-v1` explicitly) **and** they sync, through a path of their own: `sync.ts` imports `loadLists`/`replaceLists` and moves them as their own rows rather than as a blob key, which is why they are absent from `SYNC_KEYS`. So "not in any backup" was wrong and "format 2 carries it" was right but incomplete. **The stale text is in `HANDOVER.md`, not here, and is corrected in the same commit** — along with two more claims in the same bullet that had rotted: `rankd-review-dismissed-v1` is in `FILE_2` too, and `rankd-visit-v1` no longer exists as a concern because the recap it served was removed (N18) | DONE |
