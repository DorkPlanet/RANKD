...
| C13 | ~~**The profile shows full density from film two.** Every screen has a zero state; nothing has a **thin** state.~~ **LARGELY DONE 21 Aug 2026, and worth stating what closed it.** The taste chart renders nothing below three axes; `lockedShape` returns null below ten locked films and the caption says what unlocks it; the genre ring refuses below three genres; `topPeople` needs two films before somebody is a name. **Still open, and smaller than the original entry:** the odds-and-ends facts and the notes have no minimum of their own, so a library of a dozen films still gets a confident-sounding superlative | NARROWED, OPEN |
