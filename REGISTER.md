...
| P6 | **THE TYPE SCALE — an app-wide weight and size review. NEW, 21 Aug 2026, the user's call:** "it feels like not everything in the app is using that weight" [the profile's]. **Counted rather than eyeballed, and the count is the argument.** The app uses **16 distinct font sizes**, of which five — 9, 10, 11, 12, 13 — sit inside a 4px band and account for **245 of the 271 uses**. Sizes that close cannot create hierarchy; they only create inconsistency, because the reader sees one texture and the code sees five decisions. Weights are worse: **51 `font-extrabold`, 39 `font-bold`, 20 `font-semibold`, 3 `font-normal`.** When nine strings in ten are bold or heavier, nothing is emphasised. That is the whole of "everything reads at the same weight" | NEW, OPEN |
| P7 | **Why the profile is the exception, which is the thing to generalise.** It works because it actually spans the scale: a 26px display numeral against 9px small caps against 15px titles. The rest of the app lives in a 10-to-11px bold monotone, so the contrast the user likes is not a style the profile applies — it is simply the only screen with any range. **The proposed scale, six steps instead of sixteen:** display 26/32/44 (exists already) · title 19 · body 15 · secondary 13 · label 10 small-caps. Everything at 8, 9, 11, 12, 14, 17 and 21 snaps to its nearest neighbour, and the decision per string is which ROLE it has, not which pixel it had. **Scope honestly: this is mechanical but wide** — ~271 sites across every screen, and `DuelScreen` alone holds 38 of them. It should be done screen by screen with the user looking, not in one sweep | NEW, OPEN |
