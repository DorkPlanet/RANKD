...
| P14 | **Swipe filters on the list — all / locked / shuffled / year / genre.** The user's idea, 21 Aug: the same tab strip the profile uses, applied to the list. **One thing to settle before building it, because the five named tabs are not the same KIND of thing.** `all`, `locked` and `shuffled` are FILTERS — they hide rows and the order is untouched. `year` and `genre` are GROUPINGS — every row stays and the sections change. Putting both in one strip means a control where two tabs do something structurally different from the other three, which is the sort of thing that reads fine in a list of five words and confusing in the hand.<br><br>**The likely answer is that both belong, but not in the same strip:** filters as the swipe tabs (all/locked/shuffled, which is exactly the three states the legend already names and colours — the tabs would be the legend, made tappable), and grouping as an extension of the existing **Jump** control, which already exists for moving around the list by tier.<br><br>**The constraint that decides the cost:** the list is grouped by TIER today, and `ROW_H = 96` drives both the section spacers and the tier-jump offsets. Filtering only changes which rows exist, so it is cheap. **Regrouping by year or genre rebuilds the geometry those offsets are computed from**, which is the same load-bearing thing the type pass had to work around. Do the filters first; they are nearly free and they are the half the legend already argues for | NEW, OPEN |
...
| P16 | **Pin favourite directors and actors to the Your results tab.** The user's ask, 21 Aug. Sits directly with **P12** (pin cards to the profile) — same gesture, same surface, same storage question — and with **P11** (gold highlights should open their films). All three are the profile turning from something you read into something you arrange. `topPeople` already derives the candidates and `PersonSheet` already opens one, so the missing pieces are storage on the profile and a picker. <br><br>**DISTINCTIVENESS — and the line is much narrower than it was first written.** An earlier version of this note said that any hand-picked thing pinned near the top of a film profile lands on Letterboxd's FAVORITES block. **That was over-cautious and it was corrected by pushback.** Pinning favourites to a profile is not ownable and never was — MySpace's Top 8, Last.fm, Spotify, Steam, Goodreads all predate or parallel it. Ideas are not protectable; only expression is.<br><br>**What is actually off-limits is narrow:** their artwork and logos, the word FAVORITES over a four-poster grid in the same slot under the bio, their palette and type as a package. That is trade dress, and it needs a reader to plausibly confuse the two apps — which navy-and-gold against near-black-and-green does not.<br><br>**Everything else here is fair, and P16 is not even the same feature:** Letterboxd favourites FILMS, as posters. Directors and actors in a text list is content Letterboxd has no equivalent for. That is differentiation, not imitation. The question was raised with two screenshots side by side and the answer was that the layout is a convention nobody owns, but that THESE two entries are where the real resemblance would come from | NEW, OPEN |
